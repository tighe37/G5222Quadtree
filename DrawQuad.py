...
def drawLines(q,plt,xmin=-100,xmax=100,ymin=-100,ymax=100):
    if((q==None)or q.nw==None and q.ne==None and q.sw==None and q.se==None):
        return #if the tree being examined is a  node, don't draw anything
##    print("xmin:%f  xmax:%f  ymin:%f  ymax:%f"%(xmin,xmax,ymin,ymax))
    plt.plot([xmin,xmax],[q.point.y,q.point.y],'k-',lw=1)#draws horizontal line
    plt.plot([q.point.x,q.point.x],[ymin,ymax],'k-',lw=1)#draws vertical line
...
def project3(data):
    ...
    q = pointquadtree(points)#make quadtree
    ...
    plt.scatter(x,y,edgecolor='none',facecolor='blue',alpha=1) #plot points
    for p in points:#set mins and maxes
        if(p.x>xmax):
            xmax=p.x
        if(p.x<xmin):
            xmin=p.x
...
for i in range(10):
    ...
    data = [Point(random.random()*15.0,random.random()*15) for i in range(7)]
    project3(data)
